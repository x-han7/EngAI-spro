<template>
  ...
  <el-form ref="formRef" :model="form" :rules="rules" class="space-y-6">
    ...
    <el-form-item prop="password">
      <el-input v-model="form.password" type="password" placeholder="请输入密码" size="large" class="h-12" :prefix-icon="Lock" show-password />
    </el-form-item>
...
import { useRoute, useRouter } from "vue-router"; // 1. 引入路由钩子
const { hide } = useLogin();
...
const route = useRoute(); // 2. 获取当前路由信息（包含 query 参数）
const router = useRouter(); // 3. 获取路由器实例
